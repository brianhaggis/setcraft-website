...
def build_sitemap():
    ...
    It used to be hand-maintained, so it went stale (pages added after it was
    written were missing, and every lastmod said the same date whatever had
    changed). Generating it here means it refreshes on every release, since
    this script already runs then. lastmod comes from each file's mtime.
    """
    # Clean URL -> file. Only pages worth indexing: the beta page is unlisted,
    # 404 is not a destination, and release-notes is Sparkle's own view.
    pages = {
        "": "index.html",
    ...
    for slug, filename in sorted(pages.items()):
        ...
        lines.append(f'  <url><loc>https://getsetcraft.com/{slug}</loc><lastmod>{mod}</lastmod></url>')
        written += 1
    # Any published help articles (generated by tools/build_help_pages.py).
    help_dir = PUBLIC / "help"
    if help_dir.is_dir():
        for f in sorted(help_dir.glob("*.html")):
            mod = datetime.date.fromtimestamp(f.stat().st_mtime).isoformat()
            lines.append(f'  <url><loc>https://getsetcraft.com/help/{f.stem}</loc><lastmod>{mod}</lastmod></url>')
            written += 1
    lines.append('</urlset>')
...
def main():
    ...
    n = build_sitemap()
    print(f"Wrote {PUBLIC / 'sitemap.xml'} ({n} URLs)")
